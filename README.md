...
* Then we will utilize **Evolutionary Algorithms** to determine which encryption mapping(substitution mapping in this case) is the best suited for decryption. Basically, we will have a pool of mapping(which will be mutated via swaping) initially, take the best 5 suited mappings and then create offsprings(3 per each parent, so total remains at 20). We will again calculate the 5 best suited mappings(via log-likelihood) and repeat this process for a fixed amount of iterations.
